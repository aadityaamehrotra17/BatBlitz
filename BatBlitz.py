...
def save_game(name, score, bx, by, obstacle_x1, obstacle_x2, hole1, hole2, hole3, hole4, retro):
    ...
    with open('files/save.dat', 'wb') as f:      #opening the file in write binary mode
        pickle.dump(state, f)
...
def load_game():
    ...
    if is_file_empty('files/save.dat'):      #checking if file is empty
        buttonload.config(text="No Data", state=DISABLED)       #changing the text and state of the load button
    else:
        with open('files/save.dat', 'rb') as f:      #opening the file in read binary mode
...
screen.iconbitmap('images/avatar.ico')
...
img = PhotoImage(file="images/bat.gif")        #bat image
cloud1 = PhotoImage(file="images/cloud.gif")       #cloud image
cloud2 = PhotoImage(file="images/cloud.gif")       #cloud image
bg = PhotoImage(file="images/background.gif")      #background image for bosskey
...
controller_image = Image.open("images/controller.png")     #opening the controller image
...
def restart_game():
    ...
    img = PhotoImage(file="images/bat.gif")
    cloud1 = PhotoImage(file="images/cloud.gif")
    cloud2 = PhotoImage(file="images/cloud.gif")
...
def leaderboard(name, score):
    ...
    with open('files/leaderboard.txt', 'r') as file:     #opening the leaderboard file in read mode
    ...
    with open('files/leaderboard.txt', 'w') as file:     #opening the leaderboard file in write mode
...
def bosskey(event=None):
    ...
    bg = PhotoImage(file="images/background.gif")      #adding the background image
